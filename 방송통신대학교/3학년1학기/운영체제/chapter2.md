...
- 운영체제로부터 자원을 할당 받아 동작
  - 자원
    - 메모리 할당받고, 실행에 필요한 여러 가지 자원을 할당받은 후 마지막으로 CPU를 할당받음
    - CPU
      - 마지막 단계에서 할당 받음
      - CPU가 준비된 프로그램의 명령어를 실행시킴
    - 메모리
    - 입출력장치
    - 파일 등
  - 동작
...
- 작업관리자 항목 살펴보기
  ...
  - PID
    ...
    - 사용자 이름
      - 사용자 이름이 나와있는 건 사용자가 실행시킨 응용 프로세스
      - SYSTEM이라고 나와있는 건 시스템 프로세스

  - 메모리
    - 프로그램보다 프로세스의 사이즈가 더 크다.
      - 프로그램 + 부가적인 데이터가 필요하기 때문에 메모리에서 차지하는 공간이 커지게 된다.

...
이처럼 컴포넌트를 전환할 때 vue 인스턴스는 소멸, 생성을 반복한다. 컴포먼트의 전환마다 페이지를 초기화되어야 한다면 문제가 없지만 만약 컴포넌트 안의 값을 유지해야 하거나 성능상의 이유로 리렌더링을 피하고 싶을 때 `Keep-alive` 컴포넌트 사용할 수 있다. keep-alive 로 둘러싼 컴포넌트는 컴포넌트가 최초 생성되는 시점에 **캐시** 해둔다.
